...
function expandFields(fields) {
  ...
  fields.forEach((field, key) => {
    const fieldName = field.name; // If the field name contains a dot, it means we need to
    ...
    if (fieldName.includes('.')) {
      ...
      const nameParts = fieldName.split('.'); // Getting first part of the name for the new field

      const groupFieldName = nameParts[0]; // Put back together the parts again for the new field name

      const nestedFieldName = nameParts.slice(1).join('.'); // keep all properties of the original field, but give it the shortened name
...
function dedupFields(fields) {
  ...
  fields.forEach(field => {
    ...
    if (found) {
      ...
      if ( // only merge if found is a group and field is object, nested, or group.
      ...
      found.type === 'group' && (field.type === 'object' || field.type === 'nested' || field.type === 'group') || // as part of the loop we will be marking found.type as group-nested so found could be group-nested if it was
      ...
      (found.type === 'object' || found.type === 'nested' || found.type === 'group-nested') && field.type === 'group') {
        ...
        Object.assign(found, importantFieldProps); // if `field.type` wasn't group object or nested, then there's a conflict in types, so lets ignore it
      } else {// only `group`, `object`, and `nested` fields can be merged in this way
...
const loadFieldsFromYaml = async (pkg, datasetName) => {
  // Fetch all field definition files
  const fieldDefinitionFiles = await (0, _assets.getAssetsData)(pkg, isFields, datasetName);
  return fieldDefinitionFiles.reduce((acc, file) => {
    // Make sure it is defined as it is optional. Should never happen.
    if (file.buffer) {
      const tmpFields = (0, _jsYaml.safeLoad)(file.buffer.toString()); // safeLoad() returns undefined for empty files, we don't want that
