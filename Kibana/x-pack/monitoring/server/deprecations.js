...
const deprecations = ({
  ...
  renameFromRoot('xpack.monitoring.max_bucket_size', 'monitoring.ui.max_bucket_size', {
    ...
    level: 'warning'
  }), renameFromRoot('xpack.monitoring.show_license_expiration', 'monitoring.ui.show_license_expiration', {
    level: 'warning'
  }), renameFromRoot('xpack.monitoring.ui.container.elasticsearch.enabled', 'monitoring.ui.container.elasticsearch.enabled', {
    level: 'warning'
  }), renameFromRoot('xpack.monitoring.ui.container.logstash.enabled', 'monitoring.ui.container.logstash.enabled', {
    level: 'warning'
    ...
    level: 'warning'
  }), renameFromRoot('xpack.monitoring.elasticsearch.logFetchCount', 'monitoring.ui.elasticsearch.logFetchCount', {
    level: 'warning'
  }), renameFromRoot('xpack.monitoring', 'monitoring', {
    level: 'warning'
  }), (config, fromPath, addDeprecation) => {
    const emailNotificationsEnabled = (0, _lodash.get)(config, 'cluster_alerts.email_notifications.enabled');

    if (emailNotificationsEnabled && !(0, _lodash.get)(config, _constants.CLUSTER_ALERTS_ADDRESS_CONFIG_KEY)) {
      addDeprecation({
        ...
        correctiveActions: {
          manualSteps: [`Add [${fromPath}.${_constants.CLUSTER_ALERTS_ADDRESS_CONFIG_KEY}] to your kibana configs."`]
        },
        level: 'critical'
      });
    }
    ...
    level: 'warning'
  }), unused('cluster_alerts.allowedSpaces', {
    level: 'warning'
  }), unused('monitoring.ui.metricbeat.index', {
    level: 'warning'
