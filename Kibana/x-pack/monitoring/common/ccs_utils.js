...
exports.getConfigCcs = getConfigCcs;
exports.parseCrossClusterPrefix = parseCrossClusterPrefix;
exports.prefixIndexPattern = prefixIndexPattern;
/*
...
function prefixIndexPattern(config, indexPattern, ccs) {
  ...
  const prefixedPattern = patterns.map(pattern => `${ccs}:${pattern}`).join(','); // if a wildcard is used, then we also want to search the local indices
...
function parseCrossClusterPrefix(indexName) {
