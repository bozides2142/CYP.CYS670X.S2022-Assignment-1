...
const {
  argv
} = yargs(process.argv.slice(2)).option('kibana-install-dir', {
  default: '',
  type: 'string',
  ...
  default: false,
  type: 'boolean',
  description: 'Run all tests (an instance of Elasticsearch and kibana are needs to be available)'
}).option('grep', {
  alias: 'spec',
  default: false,
  type: 'string',
  description: 'Specify the spec files to run (use doublequotes for glob matching)'
}).option('open', {
  default: false,
  type: 'boolean',
