...
const INTERVAL_STRING_RE = new RegExp('^([0-9]*)\\s*(' + _datemath.default.units.join('|') + ')$'); // moment.js is only designed to allow fractional values between 0 and 1
// for units of hour or less.

const SUPPORT_ZERO_DURATION_UNITS = ['ms', 's', 'm', 'h']; // List of time units which are supported for use in Elasticsearch durations
...
const SUPPORT_ES_DURATION_UNITS = ['ms', 's', 'm', 'h', 'd']; // Parses an interval String, such as 7d, 1h or 30m to a moment duration.
...
function parseInterval(interval, checkValidEsUnit = false) {
  ...
  try {
    ...
    const unit = matches[2]; // In line with moment.js, only allow zero value intervals when the unit is less than 'day'.
    ...
    if (isNaN(value) || value < 1 && SUPPORT_ZERO_DURATION_UNITS.indexOf(unit) === -1 || checkValidEsUnit === true && SUPPORT_ES_DURATION_UNITS.indexOf(unit) === -1) {
      return null;
    }

    return (0, _moment.duration)(value, unit);
  } catch (e) {
    return null;
  }
}