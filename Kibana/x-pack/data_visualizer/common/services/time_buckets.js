...
function timeBucketsCalcAutoIntervalProvider() {
  ...
  const roundingRules = [[d(500, 'ms'), d(100, 'ms')], [d(5, 'second'), d(1, 'second')], [d(10, 'second'), d(5, 'second')], [d(15, 'second'), d(10, 'second')], [d(30, 'second'), d(15, 'second')], [d(1, 'minute'), d(30, 'second')], [d(5, 'minute'), d(1, 'minute')], [d(10, 'minute'), d(5, 'minute')], [d(15, 'minute'), d(10, 'minute')], [d(30, 'minute'), d(10, 'minute')], [d(1, 'hour'), d(30, 'minute')], [d(2, 'hour'), d(1, 'hour')], [d(4, 'hour'), d(2, 'hour')], [d(6, 'hour'), d(4, 'hour')], [d(8, 'hour'), d(6, 'hour')], [d(12, 'hour'), d(8, 'hour')], [d(24, 'hour'), d(12, 'hour')], [d(2, 'd'), d(1, 'd')], [d(4, 'd'), d(2, 'd')], [d(1, 'week'), d(4, 'd')], //[ d(2, 'week'), d(1, 'week') ],
  //[ d(1, 'month'), d(2, 'week') ],
  ...
  function find(rules, check, last) {
    ...
    return function (buckets, duration) {
      ...
      if (interval) {
        return _moment.default.duration(interval._data);
      }
    };
  }
...
const unitsDesc = _datemath.default.unitsDesc; // Index of the list of time interval units at which larger units (i.e. weeks, months, years) need
...
function TimeBuckets(timeBucketsConfig, fieldFormats) {
  this._timeBucketsConfig = timeBucketsConfig;
  this._fieldFormats = fieldFormats;
  this.barTarget = this._timeBucketsConfig[_common2.UI_SETTINGS.HISTOGRAM_BAR_TARGET];
  this.maxBars = this._timeBucketsConfig[_common2.UI_SETTINGS.HISTOGRAM_MAX_BARS];
}
/**
...
TimeBuckets.prototype.setBounds = function (input) {
  ...
  const moments = (0, _lodash.sortBy)(bounds.map((0, _lodash.ary)(_moment.default, 1)), Number);
...
TimeBuckets.prototype.getDuration = function () {
  if (!this.hasBounds()) return;
  return _moment.default.duration(this._ub - this._lb, 'ms');
};
/**
...
TimeBuckets.prototype.getInterval = function () {
  ...
  return decorateInterval(maybeScaleInterval(readInterval()), duration); // either pull the interval from state or calculate the auto-interval
  ...
  function maybeScaleInterval(interval) {
    ...
    let scaled; // If the number of buckets we got back from using the barTarget is less than
    ...
    return (0, _lodash.assign)(scaled, {
      preScaled: interval,
...
TimeBuckets.prototype.getIntervalToNearestMultiple = function (divisorSecs) {
  ...
  let nearestMultiple = remainder > divisorSecs / 2 ? intervalSecs + divisorSecs - remainder : intervalSecs - remainder;
  nearestMultiple = nearestMultiple === 0 ? divisorSecs : nearestMultiple;

  const nearestMultipleInt = _moment.default.duration(nearestMultiple, 'seconds');

  decorateInterval(nearestMultipleInt, this.getDuration()); // Check to see if the new interval is scaled compared to the original.
...
TimeBuckets.prototype.getScaledDateFormatter = function () {
  ...
  const DateFieldFormat = fieldFormats.getType(_common.FIELD_FORMAT_IDS.DATE);
...
function decorateInterval(interval, originalDuration) {
  ...
  interval.overflow = originalDuration > interval ? _moment.default.duration(interval - originalDuration) : false;
...
function getBoundsRoundedToInterval(bounds, interval, inclusiveEnd = false) {
  ...
  let adjustedMaxMs = Math.ceil(bounds.max.valueOf() / intervalMs) * intervalMs; // Don't include the start ms of the next bucket unless specified..
