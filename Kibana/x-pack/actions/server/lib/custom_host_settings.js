...
exports.getCanonicalCustomHostUrl = getCanonicalCustomHostUrl;
exports.resolveCustomHosts = resolveCustomHosts;
...
function getCanonicalCustomHostUrl(url) {
...
const ProtocolsForSmtp = new Set(['smtp:']); // converts the custom host data in config, for ease of use, and to perform
// validation we can't do in config-schema, since the cloud validation can't
// do these sorts of validations

function resolveCustomHosts(logger, config) {
  ...
  for (const customHostSetting of result.customHostSettings) {
    ...
    if (parsedUrl.username || parsedUrl.password) {
      logger.warn(`${ErrorPrefix} URL "${originalUrl}" contains authentication information which will be ignored, but should be removed from the configuration`);
    }

    if (parsedUrl.hash) {
      logger.warn(`${ErrorPrefix} URL "${originalUrl}" contains hash information which will be ignored`);
    }

    if (parsedUrl.pathname && parsedUrl.pathname !== '/') {
      logger.warn(`${ErrorPrefix} URL "${originalUrl}" contains path information which will be ignored`);
    }

    if (!ProtocolsForSmtp.has(parsedUrl.protocol) && customHostSetting.smtp) {
      logger.warn(`${ErrorPrefix} URL "${originalUrl}" contains smtp properties but does not use smtp; ignoring smtp properties`);
    ...
    if (customHostSetting.ssl) {
      var _customHostSetting$ss;

      let files = ((_customHostSetting$ss = customHostSetting.ssl) === null || _customHostSetting$ss === void 0 ? void 0 : _customHostSetting$ss.certificateAuthoritiesFiles) || [];
    ...
    if (customSmtpSettings) {
      if (customSmtpSettings.requireTLS && customSmtpSettings.ignoreTLS) {
        logger.warn(`${ErrorPrefix} URL "${originalUrl}" cannot have both requireTLS and ignoreTLS set to true; using requireTLS: true and ignoreTLS: false`);
  ...
  for (const customHostSetting of savedSettings) {
    ...
    if (existingUrls.has(url)) {
      logger.warn(`${ErrorPrefix} multiple URLs match the canonical url "${url}"; only the first will be used`); // mark this entry to be able to delete it after processing them all
...
function appendToCertificateAuthoritiesData(customHost, cert) {
...
function getFileContents(logger, fileName) {
  try {
    return (0, _fs.readFileSync)(fileName, 'utf8');
  } catch (err) {
    logger.warn(`error reading file "${fileName}" specified in xpack.actions.customHosts, ignoring: ${err.message}`);
    return;
  }
} // 0 isn't a valid port, so result can be checked as falsy


function getActualPort(protocol, port) {
  if (port !== '') {
