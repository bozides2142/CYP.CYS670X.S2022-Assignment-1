...
function registerCollector(usageCollection, kibanaIndex) {
  const collector = usageCollection.makeUsageCollector({
    ...
    fetch: async ({
          ...
          query: {
            bool: {
              filter: [{
                ...
                term: {
                  'alert.alertTypeId': _alerts.ML_ALERT_TYPES.AD_JOBS_HEALTH
                }
              }]
            }
          }
      ...
      const resultsByCheckType = jobsHealthRuleInstances.body.hits.hits.reduce((acc, curr) => {
        ...
        const resultConfig = (0, _alerts2.getResultJobsHealthRuleConfig)(testsConfig);
      ...
      return {
        alertRules: {
          ...
          [_alerts.ML_ALERT_TYPES.AD_JOBS_HEALTH]: {
