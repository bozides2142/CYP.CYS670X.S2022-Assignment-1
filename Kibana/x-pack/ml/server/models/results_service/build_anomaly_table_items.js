...
function buildAnomalyTableItems(anomalyRecords, aggregationInterval, dateFormatTz) {
  ...
  return displayRecords.map((record, index) => {
    ...
    const jobId = source.job_id; // Identify each row with a unique ID which is used by the table for row expansion.
...
function aggregateAnomalies(anomalyRecords, interval, dateFormatTz) {
  ...
  anomalyRecords.forEach(record => {
    ...
    const entitiesForDetector = detectorsForJob[detectorIndex]; // TODO - are we worried about different byFields having the same
