...
exports.LEGACY_LAST_MODIFIED_VERSION = exports.FILEBEAT_7X_INDICATOR_PATH = void 0;
exports.getMigrations = getMigrations;
exports.isSiemSignalsRuleType = exports.isSecuritySolutionLegacyNotification = exports.isDetectionEngineAADRuleType = exports.isAnyActionSupportIncidents = void 0;
...
const isAnyActionSupportIncidents = doc => doc.attributes.actions.some(action => SUPPORT_INCIDENTS_ACTION_TYPES.includes(action.actionTypeId)); // Deprecated in 8.0
...
function getMigrations(encryptedSavedObjects, isPreconfigured) {
  ...
  const migrationSecurityRules713 = createEsoMigration(encryptedSavedObjects, doc => isSiemSignalsRuleType(doc), pipeMigrations(removeNullsFromSecurityRules));
  const migrationSecurityRules714 = createEsoMigration(encryptedSavedObjects, doc => isSiemSignalsRuleType(doc), pipeMigrations(removeNullAuthorFromSecurityRules));
  const migrationSecurityRules715 = createEsoMigration(encryptedSavedObjects, doc => isSiemSignalsRuleType(doc), pipeMigrations(addExceptionListsToReferences));
  ...
  const migrationRules800 = createEsoMigration(encryptedSavedObjects, doc => true, pipeMigrations(addThreatIndicatorPathToThreatMatchRules, addSecuritySolutionAADRuleTypes, fixInventoryThresholdGroupId));
  const migrationRules801 = createEsoMigration(encryptedSavedObjects, doc => true, pipeMigrations(addSecuritySolutionAADRuleTypeTags));
  return {
...
function addSecuritySolutionAADRuleTypes(doc) {
  ...
  return isSiemSignalsRuleType(doc) && (0, _securitysolutionRules.isRuleType)(ruleType) ? { ...doc,
    attributes: { ...doc.attributes,
      alertTypeId: _securitysolutionRules.ruleTypeMappings[ruleType],
      enabled: false,
      params: { ...doc.attributes.params,
        outputIndex: ''
      }
    }
  } : doc;
...
function addSecuritySolutionAADRuleTypeTags(doc) {
  ...
  const ruleType = doc.attributes.params.type;
  return isDetectionEngineAADRuleType(doc) && (0, _securitysolutionRules.isRuleType)(ruleType) ? { ...doc,
    attributes: { ...doc.attributes,
...
function addThreatIndicatorPathToThreatMatchRules(doc) {
  ...
  return isSiemSignalsRuleType(doc) && ((_doc$attributes$param = doc.attributes.params) === null || _doc$attributes$param === void 0 ? void 0 : _doc$attributes$param.type) === 'threat_match' && !doc.attributes.params.threatIndicatorPath ? { ...doc,
    attributes: { ...doc.attributes,
      params: { ...doc.attributes.params,
