...
run(async ({
  ...
  const clean = () => {
    ...
    del.sync([SHAREABLE_RUNTIME_FILE], {
      force: true
    });
  };
  ...
  if (flags.run) {
    ...
    execa.sync('yarn', ['webpack-dev-server', '--config', webpackConfig, ...(process.stdout.isTTY && !process.env.CI ? ['--progress'] : []), '--hide-modules', '--display-entrypoints', 'false', '--content-base', SHAREABLE_RUNTIME_SRC], options);
    return;
  }

  if (flags.stats) {
    log.info('Writing Webpack stats...');
    const output = execa(require.resolve('webpack/bin/webpack'), ['--config', webpackConfig, '--profile', '--json'], { ...options,
      env,
  ...
  execa.sync('yarn', ['webpack', '--config', webpackConfig, '--hide-modules', ...(process.stdout.isTTY && !process.env.CI ? ['--progress'] : [])], { ...options,
    env
  });
