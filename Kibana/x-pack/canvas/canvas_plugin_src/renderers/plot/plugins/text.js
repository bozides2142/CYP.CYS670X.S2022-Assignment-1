...
function
/*plot, options*/
processOptions() {// Nothing
}

function draw(plot, ctx) {
  (0, _jquery.default)('.valueLabel', plot.getPlaceholder()).remove();
  plot.getData().forEach(function (series) {
    const show = (0, _lodash.get)(series.numbers, 'show');

    if (!show) {
      return;
    }

    let points = series.data; // TODO: This might only work on single x and y axis charts.

    if (series.stack != null) {
      points = points.map((point, i) => {
        const p = point.slice(0); // This magic * 3 and + 1 are due to the way the stacking plugin for flot modifies the series.
    ...
    function writeText(text, x, y) {
      ...
      const textNode = (0, _jquery.default)('<div/>').text(String(text)).addClass('valueLabel').css({
        position: 'absolute'
      });
