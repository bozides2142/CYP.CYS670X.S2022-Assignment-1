...
const REDRAW_ATTEMPTS = 10; // Factor by which to shrink the pie when fitting labels within the plot
...
function init(plot) {
  ...
  let centerTop = null;
  let processed = false;
  let ctx = null; // interactive variables

  let highlights = []; // add hook to determine if pie plugin in enabled, and then perform necessary operations
  ...
  function processDatapoints(plot) {
    if (!processed) {
      ...
      target = (0, _jquery.default)(canvas).parent();
  ...
  function combine(data) {
    ...
    const newdata = []; // Fix up the raw data from Flot, ensuring the data is numeric

    for (let i = 0; i < data.length; ++i) {
      let value = data[i].data; // If the data is an array, we'll assume that it's a standard
    ...
    for (let i = 0; i < data.length; ++i) {
      ...
      if (numCombined < 2 || value / total > options.series.pie.combine.threshold) {
        newdata.push(_jquery.default.extend(data[i], {
  ...
  function draw(plot, newCtx) {
    ...
    let attempts = 0; // Keep shrinking the pie's radius until drawPie returns true,
    ...
    function drawPie() {
      ...
      const radius = options.series.pie.radius > 1 ? options.series.pie.radius : maxRadius * options.series.pie.radius; // center and rotate to starting position
      ...
      function drawLabels() {
        ...
        function drawLabel(slice, startAngle, index) {
          ...
          if (options.series.pie.label.background.opacity !== 0) {
            ...
            (0, _jquery.default)("<div class='pieLabelBackground' style='position:absolute;width:" + label.width() + 'px;height:' + label.height() + 'px;' + pos + 'background-color:' + c + ";'></div>").css('opacity', options.series.pie.label.background.opacity).insertBefore(label);
          }
    ...
    for (let i = 0; i < slices.length; ++i) {
      ...
      if (s.pie.show) {
        ...
        if (ctx.isPointInPath) {
          ...
          const arrPoint = [x, y]; // TODO: perhaps do some mathmatical trickery here with the Y-coordinate to compensate for pie tilt?
