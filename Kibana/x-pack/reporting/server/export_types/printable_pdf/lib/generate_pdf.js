...
const getTimeRange = urlScreenshots => {
  const grouped = (0, _lodash.groupBy)(urlScreenshots.map(({
    timeRange
  }) => timeRange));
...
function generatePdfObservable(reporting, logger, title, options, logo) {
  const tracker = (0, _tracker.getTracker)();
  tracker.startScreenshots();
  return reporting.getScreenshots(options).pipe((0, _operators.mergeMap)(async ({
    layout,
    metrics$,
    results
  }) => {
