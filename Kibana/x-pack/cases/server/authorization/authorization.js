...
class Authorization {
  ...
  static async create({
    ...
    try {
      var _await$getSpace$disab, _await$getSpace;

      const disabledFeatures = new Set((_await$getSpace$disab = (_await$getSpace = await getSpace(request)) === null || _await$getSpace === void 0 ? void 0 : _await$getSpace.disabledFeatures) !== null && _await$getSpace$disab !== void 0 ? _await$getSpace$disab : []);
      caseOwners = new Set(features.getKibanaFeatures() // get all the features' cases owners that aren't disabled
      .filter(({
        id
