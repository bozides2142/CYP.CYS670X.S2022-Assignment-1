...
const getCreateMigration = (encryptedSavedObjectsService, instantiateServiceWithLegacyType) => opts => {
  ...
  return (encryptedDoc, context) => {
    ...
    const {
      ...
      id,
      originId
    } = encryptedDoc; // If an object is slated to be converted, it should be decrypted flexibly:
    ...
    const isTypeBeingConverted = !!context.convertToMultiNamespaceTypeVersion && _semver.default.lte(context.migrationVersion, context.convertToMultiNamespaceTypeVersion); // This approximates the behavior of getDescriptorNamespace(); the intent is that if there is ever a case where a multi-namespace object
    ...
    let decryptDescriptorNamespace = descriptorNamespace; // If an object has been converted right before this migration function is called, it will no longer have a `namespace` field, but it
