...
function nearest(dataTuples, targetTuples) {
  return _lodash.default.map(targetTuples, function (bucket) {
    ...
    while (i < dataTuples.length - 1 && (Math.abs(dataTuples[i + 1][0] - time) < Math.abs(dataTuples[i][0] - time) || // TODO: Certain offset= args can cause buckets with duplicate times, e.g., offset=-1M
