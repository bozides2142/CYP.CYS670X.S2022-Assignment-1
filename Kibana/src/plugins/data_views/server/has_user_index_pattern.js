...
exports.hasUserIndexPattern = void 0;
...
const hasUserIndexPattern = async ({
  ...
  const indexPatterns = await soClient.find({
    type: 'index-pattern',
    fields: ['title'],
    search: `*`,
    searchFields: ['title'],
  ...
  const hasAnyNonDefaultFleetIndices = resolveResponse.body.indices.some(ds => ds.name !== _constants.DEFAULT_ASSETS_TO_IGNORE.METRICS_ENDPOINT_INDEX_TO_IGNORE);
  if (hasAnyNonDefaultFleetIndices) return true;
  const hasAnyNonDefaultFleetDataStreams = resolveResponse.body.data_streams.some(ds => ds.name !== _constants.DEFAULT_ASSETS_TO_IGNORE.METRICS_DATA_STREAM_TO_IGNORE && ds.name !== _constants.DEFAULT_ASSETS_TO_IGNORE.LOGS_DATA_STREAM_TO_IGNORE && ds.name !== _constants.DEFAULT_ASSETS_TO_IGNORE.ENT_SEARCH_LOGS_DATA_STREAM_TO_IGNORE);
