...
exports.areJobsCompatible = areJobsCompatible;
exports.mergeJobConfigurations = mergeJobConfigurations;
 ...
 * @returns {boolean}
 */
function areJobsCompatible(jobs = []) {
...
function mergeJobConfigurations(jobs = []) {
  if (!jobs || !Array.isArray(jobs) || !jobs.length) {
    throw new Error('No capabilities available');
  }

  const allAggs = {}; // For each job, look through all of its fields

  jobs.forEach(job => {
    const fields = job.fields;
    const fieldNames = Object.keys(fields); // Check each field

    fieldNames.forEach(fieldName => {
      const fieldAggs = fields[fieldName]; // Look through each field's capabilities (aggregations)
      ...
      fieldAggs.forEach(agg => {
        ...
        const isDateHistogramAgg = aggName === 'date_histogram'; // If we currently don't have this aggregation, add it.
        ...
        if (aggDoesntExist || fieldDoesntExist && !isDateHistogramAgg) {
          ...
          switch (aggName) {
            ...
            case 'date_histogram':
              if (fieldDoesntExist || !(0, _lodash.isEqual)(fieldAgg, agg)) {
