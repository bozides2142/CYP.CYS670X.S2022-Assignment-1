...
const encodePathname = pathname => {
  ...
  const decodedPath = (_URLSearchParams$get = new _url.URLSearchParams(`path=${pathname}`).get('path')) !== null && _URLSearchParams$get !== void 0 ? _URLSearchParams$get : '';
  return decodedPath.split('/').map(str => encodeURIComponent(str)).join('/');
