...
const model = (currentState, resW) => {
  ...
  let stateP = currentState; // Handle retryable_es_client_errors. Other left values need to be handled
  ...
  if (stateP.controlState === 'INIT') {
    ...
    if (Either.isRight(res)) {
      ...
      const aliases = (0, _helpers.getAliases)(indices);
    ...
    const res = resW; // If the write block is successfully in place
    ...
    if (Either.isRight(res)) {
      ...
      if ((0, _actions.isLeftTypeof)(res.left, 'unknown_docs_found')) {
        return { ...stateP,
          controlState: 'FATAL',
          reason: (0, _extract_errors.extractUnknownDocFailureReason)(res.left.unknownDocs, stateP.sourceIndex.value)
        };
      } else {
    ...
    if (Either.isRight(res)) {
      ...
      const logs = (0, _progress.logProgress)(stateP.logs, progress);
      ...
      if (res.right.outdatedDocuments.length > 0) {
        ...
        if (stateP.corruptDocumentIds.length > 0 || stateP.transformErrors.length > 0) {
          const transformFailureReason = (0, _extract_errors.extractTransformFailuresReason)(stateP.corruptDocumentIds, stateP.transformErrors);
          return { ...stateP,
            controlState: 'FATAL',
    ...
    const res = resW; // Increment the processed documents, no matter what the results are.
    ...
    if (Either.isRight(res)) {
      if (stateP.corruptDocumentIds.length === 0 && stateP.transformErrors.length === 0) {
        const batches = (0, _create_batches.createBatches)(res.right.processedDocs, stateP.tempIndex, stateP.maxBatchSizeBytes);
      ...
      if ((0, _actions.isLeftTypeof)(left, 'documents_transform_failed')) {
        return { ...stateP,
          controlState: 'REINDEX_SOURCE_TO_TEMP_READ',
    ...
    if (Either.isRight(res)) {
      ...
      if ((0, _actions.isLeftTypeof)(res.left, 'target_index_had_write_block') || (0, _actions.isLeftTypeof)(res.left, 'index_not_found_exception')) {
      ...
      } else if ((0, _actions.isLeftTypeof)(res.left, 'request_entity_too_large_exception')) {
        return { ...stateP,
          controlState: 'FATAL',
    ...
    if (Either.isRight(res)) {
      ...
      if ((0, _actions.isLeftTypeof)(res.left, 'documents_transform_failed')) {
    ...
    if (Either.isRight(res)) {
      ...
      if ((0, _actions.isLeftTypeof)(res.left, 'request_entity_too_large_exception')) {
        return { ...stateP,
          controlState: 'FATAL',
          reason: FATAL_REASON_REQUEST_ENTITY_TOO_LARGE
        };
      } else if ((0, _actions.isLeftTypeof)(res.left, 'target_index_had_write_block') || (0, _actions.isLeftTypeof)(res.left, 'index_not_found_exception')) {
