...
async function bootstrap({
  ...
  process.on('SIGHUP', () => reloadConfiguration()); // This is only used by the LogRotator service
  ...
  try {
    const {
      preboot
    } = await root.preboot(); // If setup is on hold then preboot server is supposed to serve user requests and we can let
    ...
    await root.setup();
    await root.start(); // Notify parent process if we haven't done that yet during preboot stage.
